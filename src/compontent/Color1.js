...
import img from "./assets/sh2.png";
...
const Color = () => {
  ...
  const [selectedRating, setSelectedRating] = useState(0);
   ...
   useEffect(() => {
    ...
    const handleHover = (color, rating) => {
      setHoverColor(color);
      setHoverRating(rating);
    };
    const handleRatingClick = (color, rating) => {
      ...
      // Example: You can make an API call here to store the rating in the database
    };
   ...
   useEffect(() => {
     fetchRatings(); // Fetch ratings data when component mounts
   }, []);
  ...
  return (
    ...
    <div className="main1">
      <div className="upload">
        <h2 style={{ color: "white" }}>Upload Pant  </h2>
  
<div>
  {/* <Checkbox {...label}  onChange={handleCheckboxChange}  />  */}
  <input style={{ transform: 'scale(1.5)', marginRight:'10px'}} type="checkbox" type="checkbox" checked={isClimateChecked} onChange={handleCheckboxChange} />
      <span style={{ color: isClimateChecked ? 'green' : 'white', fontSize: '18px' }}>Climate </span>
</div>
...
<div className="container12">
    ...
    <div className="temp-scale">
      <span>Celsius</span>
            ...
            <h2 style={{ color: "white" }}>Suggestion For Shirts</h2>
            <div className="display-swatches" style={{ marginTop: 20 }}>
  {/* {renderSwatches("hex", colorNames)} */}
  {renderSwatches("hex",  colorNames, selectedSeason)}
            {/* <div>Type of cloth: {clothType}</div> */}
          </div>
      </div>
    </div>
    </div>
